...
func apply_slot_upgrade(slot_index: int, chosen_resource: StatusEffectData):
	...
	if current_resource == chosen_resource:
		...
		print("Levelled up '%s' on slot %d to level %d" % [chosen_resource.display_name, slot_index + 1, current_data["level"]])
	else:
		current_data["resource"] = chosen_resource
		current_data["level"] = 1
		print("Applied new effect '%s' to Slot %d" % [chosen_resource.display_name, slot_index + 1])
