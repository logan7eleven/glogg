...
func _unhandled_input(event):
	if not can_move: return
	if event.is_action_pressed("ui_send_all_staging"): # R2
...
func _attempt_loot_pickup():
	...
	if nearest_drop != null and nearest_drop.stored_block != null:
		...
		# If we already have a block, eject it back into the world
		if GlobalState.master_temp_storage != null:
			var ejected_block = GlobalState.master_temp_storage
			var drop_instance = DROPPED_BLOCK_SCENE.instantiate()
			get_parent().add_child(drop_instance)
			drop_instance.global_position = global_position
			drop_instance.setup(ejected_block)
			print("Ejected old block with ", int(ejected_block.remaining_integrity), "s left")
		
		GlobalState.master_temp_storage = new_block
		nearest_drop.queue_free()

func _swap_temp_and_perm_storage():
	var hold = GlobalState.master_perm_storage
	GlobalState.master_perm_storage = GlobalState.master_temp_storage
	GlobalState.master_temp_storage = hold
